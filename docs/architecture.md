...
加えて、QU の inner loop に **search controller** と **reranker** が LLM tool として加わります（後述）。
...
定義されている prompt（version はすべて `2026-05-29.1`）：

| prompt_id | role | 場所 |
|---|---|---|
| `qu.query_planner` | planner | `qu/prompt_registry.py` |
| `qu.retrieval_controller` | retrieval_subagent | 同上 |
| `qu.reranker` | reranker | 同上 |
| `qu.answer_judge` | judge | 同上 |
| `qu.downstream_query_bootstrap` | query_bootstrapper | 同上 |
| `cu.schema_induction` | inducer | `cu/prompt_registry.py` |
| `cu.field_extraction` | extractor | 同上 |
...
### 5.1 多段 step executor

```
load_schema
  → plan_query  (LLM-first、heuristic は validation failure fallback)
  → [steps を順次実行] state を蓄積
       ...
       ├─ fetch_chunks
       └─ review_schema_gaps
  → run_search_iterations  (search step 後だけ ReAct ループ)
...
`rerank_search_results` がすべての step 終了後に LLM rerank を実行。`state["chunks"]` を上書きし、`rerank_score` / `rerank_reason` を付与。失敗時は元の順序を維持して `llm_rerank:error` を trace に積むだけ。
...
LLM judge が default で常時稼働する。`HeuristicAnswerJudge` は JSON validation failure や API failure 時の reliability fallback と smoke test helper であり、CLI/demo の agent intelligence mode ではない。LLM judge は `--judge-model / --judge-base-url / --judge-api-key` で planner とモデルを分離可能（self-bias / 共謀防止のための運用 path）。
...
1. **`column_requests`** — QU の planner / `review_schema_gaps` step が直接出す
...
`summarize_feedback` が 3 チャネルを priority-merge し、同名 field は `count` を加算。
...
| QU answer JSON | クエリ単位の結果 | answer, plan, records, aggregation, evidence, judgement, column_requests, search_diagnostics, rerank, prompt_state, trace |
...
| OBS-001 | Budget, observability, replay | Partial（orchestration_trace で骨格、latency/cost は未収録） |
...
8. **Append-only signal logs** — `column_requests.jsonl`, `prompt_repair_request.jsonl`, `orchestration_trace.jsonl` は時系列で蓄積、後段で集計
9. **Deterministic control は残す** — BM25 explainability、query diversity、forced search iteration、smoke eval は system reliability layer として維持
10. **観測可能性は構造化 JSON で出す** — trace の tool 名に backend / provider / fallback chain をすべて文字列展開
...
| ファイル | 責務 |
|---|---|
| `agent.py` | QueryUnderstandingAgent（多段 step executor） |
...
| `planner.py` | QueryPlan / QueryToolStep / ColumnRequest + Heuristic/LLM planner + 7 ALLOWED_STEP_TOOLS の validator |
...
| `prompt_registry.py` | PromptSpec(frozen=True) と QU 側 prompt 定義 |
| `prompt_repair.py` | prompt_repair_request.jsonl の生成（signal-only） |
...
| ファイル | 責務 |
|---|---|
| `qu_cu_loop_demo.py` | outer-loop orchestration runner、`query_tasks` + `orchestration_trace` を含む完全実行 |
| `qu_user_query_demo.py` | 単発ユーザクエリの実行 demo（default で agentic retrieval + OpenAI planner/reranker/judge） |

---

**最終更新**: 2026-05-30 / **schema_version**: `silver.rlm.v1` / **prompt baseline**: `2026-05-29.1`
